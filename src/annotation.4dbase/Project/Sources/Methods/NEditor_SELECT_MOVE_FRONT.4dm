...
For ($i; 1; Size of array:C274($selectDoms))
	...
	For ($j; 1; $objectCount)
		...
		If ($level<$objectCount)
			If ($level<($objectCount-2))
				If ($selectObjectId=$objectId)
					DOM_ELEMENT_MOVE_TO_LEVEL($selectDom; $level+2)
					DOM_ELEMENT_MOVE_TO_LEVEL($objectDom; $level+2)
					DOM_ELEMENT_MOVE_TO_LEVEL(DOM Find XML element by ID:C1010($Container->; $objectId); $level+2)
					$j:=$objectCount
