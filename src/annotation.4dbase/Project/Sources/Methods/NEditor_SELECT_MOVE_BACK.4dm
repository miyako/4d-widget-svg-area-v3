	...
	For ($j; 1; $objectCount)
		...
		If ($level#1)
			...
			If ($selectObjectId=$objectId)
				DOM_ELEMENT_MOVE_TO_LEVEL($selectDom; $level-1)
				DOM_ELEMENT_MOVE_TO_LEVEL($objectDom; $level-1)
				DOM_ELEMENT_MOVE_TO_LEVEL(DOM Find XML element by ID:C1010($Container->; $objectId); $level-1)
				$j:=$objectCount
