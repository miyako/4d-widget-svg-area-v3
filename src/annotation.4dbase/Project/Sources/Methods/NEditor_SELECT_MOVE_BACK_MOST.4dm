	...
	For ($j; 1; $objectCount)
		...
		If ($level#1)
			...
			If ($selectObjectId=$objectId)
				DOM_ELEMENT_MOVE_TO_LEVEL($selectDom; 1)
				DOM_ELEMENT_MOVE_TO_LEVEL($objectDom; 1)
				DOM_ELEMENT_MOVE_TO_LEVEL(DOM Find XML element by ID:C1010($Container->; $objectId); 1)
				$j:=$objectCount
