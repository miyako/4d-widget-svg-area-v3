...
For ($i; 1; Size of array:C274($selects))
	...
	If (OK=1)  //is an object with style
		...
		If (Match regex:C1019("(?:stroke-opacity:)([^;]+)"; $style; 1; $pos; $len))
			
			$thisStrokeOpacity:=Num:C11(Substring:C12($style; $pos{1}; $len{1}))
			
			If ($isUndefined)
				$strokeOpacity:=$thisStrokeOpacity
				$isUndefined:=False:C215
			Else 
				If ($thisStrokeOpacity#$strokeOpacity)
					$strokeOpacity:=0
				End if 
			End if 
