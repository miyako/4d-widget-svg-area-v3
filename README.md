...
[![license](https://img.shields.io/github/license/miyako/4d-widget-svg-area-v3)](LICENSE)
...
範囲指定でオブジェクトを選択します。また，選択されたオブジェクト群を移動またはリサイズまたは回転することができます。選択されたオブジェクトのハンドル部分をドラッグした場合はリサイズ，その他の部分をドラッグした場合は移動となります。

ドラッグ中に``shift``キーが押されている場合，リサイズ中であれば，長方形は正方形に・楕円は円に・線は正方形の範囲を横切る``45``度の斜め線に・自由線はアスペクト比を維持した自由線に変化します。移動中に``shift``キーが押されている場合，オブジェクトは移動せず，代わりにマウスポインターの位置に向かって回転します。
...
エディターには，外部アプリケーション由来のテキストおよびイメージをペーストすることができます。
...
**注記**: Macは``option``，Windowsは``Alt``キーを押しながら画像ファイルをドロップすることにより，背景（元画像）を切り替えることができます。

### 整列

オブジェクトの位置を揃えるには，2個以上のオブジェクトが選択された状態で右クリックしてコンテキストメニューから項目を選択します。

* 整列 > 左揃え | 右揃え | 上揃え | 下揃え | 水平揃え | 垂直揃え

オブジェクトを等間隔で配置するには，3個以上のオブジェクトが選択された状態で右クリックしてコンテキストメニューから項目を選択します。

* 整列 > 水平等間隔揃え | 垂直等間隔揃え

### 順序

オブジェクトの前後関係を変更するには，右クリックしてコンテキストメニューから項目を選択します。

* 順序 > 最前面へ移動 | 最背面へ移動 | 前面へ移動 | 背面へ移動

### ツールボタンについて

フォーム「Editor」には，描画色・線幅・背景色・描画色透明度・背景色透明度・順序・整列それぞれの標準ツールボタンと，いくつかのツールをまとめたポップアップ版のツールボタンが配置されています。個別のボタンは，表示エリアの外に隠されており，エリア内に表示されるのは，ポップアップ版のほうです。どちらのボタンもプログラム的には動作しており，両者を同時に使用することもできます。ボタンの位置やサイズなどは自由にカスタマイスすることができますが，オブジェクト名は，ボタンのアクティブ状態を制御するために使用されていますので，変更はしないでください。

それぞれのボタンにはヘルプTipが設定されており，マウスポインターを重ねると，数秒間，その役割が文字列で表示されます。ボタンのラベルテキストが必要であれば，フォーム「Editor」にスタティックテキストオブジェクトを追加してください。

### エリアのズームについて

エリアをズーム表示した場合，マウスの上下左右スクロールボタンやトラックパッドで表示エリアを動かすことができます。スクロールバーは，表示されないデフォルト設定になっています。必要に応じてスクロールバーが表示されるようにしたければ，下記のようにフォームメソッドを書き換えてください。
...
NEditor_SET_SCROLLBAR_VISIBLE ("Editor";True)
...
```
NEditor_SET_SCROLLBAR_VISIBLE (area;visible)
visible:=NEditor_Get_scrollbar_visible (area)
```

Parameter|Type|Description
------------|------------|----
area|TEXT|エディターのオブジェクト名
visible|BOOLEAN|必要に応じてスクロールバーを表示する（標準: ``False``）
...
画像は，``SVG``にエディター固有の情報（``editor``名前空間）が埋め込まれたものが返されます。ピクチャなので，そのままフィールドに表示したり，データベースに保存することができます。エディターで編集した画像をデータベースなどに保存し，後で編集を継続することを想定しています。


``1``

画像は，``SVG``にエディター固有の情報（``editor``名前空間）が埋め込まれたものが返されます。``0``と違い，エディター上の選択状態は解除され，元画像がオリジナルサイズ（回転されている場合は枠内に収まるサイズ）でエクスポートされます。``WRITE PICTURE FILE``や``CONVERT PICTURE``で一般的な``PNG``形式などに変換することを想定しています。
